...
set @BegDate='2017-09-16'
set @EndDate='2017-10-04'
set @Exchange=9.0

-------------------------------------
DECLARE @WeeklyReport_0 table(ProductId int,Paymenttime datetime, PurchaseOrderNumber varchar(max), ProductCode varchar(max), Brand nvarchar(max), ProductName nvarchar(max),  Payment decimal(18,2), DiscountFee decimal(18,2), OrderPayment decimal(18,2), Quantity int, StatusId int, MallCode varchar(max), buyerId varchar(max), Cost decimal(18,2), TrackingNumber varchar(max),OriginalProductCode nvarchar(max) )
...
DECLARE @OriginalProductCode nvarchar(max)
...
INTO @ProductId, @Paymenttime,  @PurchaseOrderNumber, @ProductCode, @Brand, @ProductName, @Payment, @DiscountFee, @OrderPayment, @Quantity, @StatusId, @MallCode, @buyerId, @Cost, @TrackingNumber,@OriginalProductCode
...
--	end
    ...
    insert into @WeeklyReport_0 values(@ProductId,@Paymenttime,  @PurchaseOrderNumber, @ProductCode, @Brand,@ProductName, @Payment, @DiscountFee, @OrderPayment, @Quantity, @StatusId, @MallCode, @buyerId, @Cost, @TrackingNumber,@OriginalProductCode)
...
SELECT WRJ.Paymenttime,  WRJ.PurchaseOrderNumber, WRJ.ProductCode, WRJ.Brand, WRJ.ProductName, WRJ.Payment, WRJ.DiscountFee, WRJ.OrderPayment, WRJ.Quantity, WRJ.StatusId, WRJ.MallCode, PI.buyerId, WRJ.Cost, WRJ.TrackingNumber,WRJ.OriginalProductCode
into #temp500
FROM #Report_0  WRJ
...
Insert into #temp500 
select Paymenttime, PurchaseOrderNumber, ProductCode, Brand, ProductName, Payment, DiscountFee, OrderPayment, Quantity, StatusId, MallCode, buyerId, Cost, TrackingNumber,OriginalProductCode from #temp600
--select * from #temp500 
...
into #Report_1 from #temp500
 ...
 select [Paymenttime]
	...
	-- select [Brand],sum(Revenue) as sums from #revenue_temp1 group by [Brand] order by sums desc
