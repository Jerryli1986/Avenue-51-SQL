/****** Script for SelectTopNRows command from SSMS  ******/
  ...
  ---如果计算16年9月前的数据就需要特别注意了 (10,12,23,101)
...
SELECT  
	  ...
	  ,Tb.[PaidDateUtc]
	  ,Tb.Suppliers
	  ,case when Tb.[ReferenceOrderId] like '%OWE%' then '' else Tb.ShippedHours end as ShippedHours 
	  ,Tb.months
	  ,Tb.weeks
  into #temp1000
  FROM 
   (SELECT 
		...
		,s.[CreatedOnUtc]
		 ,s.TrackingNumber 
		 ,CASE when s.[ReferenceOrderId] like '%OWE%' Then 'OWE'
		       WHEN s.[ReferenceOrderId] like '%ORD%' and s.[TrackingNumber]=''  THEN 'WAIT' 
		       when s.[ReferenceOrderId] like '%ORD%' and substring(s.TrackingNumber,1,2)='BE' then substring(s.TrackingNumber,1,2)+reverse(substring(reverse(s.TrackingNumber),1,2)) 
			   when s.[ReferenceOrderId] like '%ORD%' and substring(s.TrackingNumber,1,2)='EA' then substring(s.TrackingNumber,1,2)+reverse(substring(reverse(s.TrackingNumber),1,2))
			   when s.[ReferenceOrderId] like '%ORD%' and substring(s.TrackingNumber,1,2)='EK' then substring(s.TrackingNumber,1,2)+reverse(substring(reverse(s.TrackingNumber),1,2))
			   Else 'OTHERS'
			   END as Suppliers
		, case when datepart(dw,PaidDateUtc)=2  and DATEDIFF(day, [PaidDateUtc], S.ShippingDate)>=7 then DATEDIFF(HOUR, dateadd(hh,48,S.[PaidDateUtc]),  S.ShippingDate)  
	         when datepart(dw,[PaidDateUtc])=3  and DATEDIFF(day, [PaidDateUtc], S.ShippingDate)>=6 then DATEDIFF(HOUR, dateadd(hh,48,S.[PaidDateUtc]),  S.ShippingDate) 
	         when datepart(dw,[PaidDateUtc])=4  and DATEDIFF(day, [PaidDateUtc], S.ShippingDate)>=5 then DATEDIFF(HOUR, dateadd(hh,48,S.[PaidDateUtc]),  S.ShippingDate)  
	         when datepart(dw,[PaidDateUtc])=5  and DATEDIFF(day, [PaidDateUtc], S.ShippingDate)>=4 then DATEDIFF(HOUR, dateadd(hh,48,S.[PaidDateUtc]),  S.ShippingDate)  
	         when datepart(dw,[PaidDateUtc])=6  and DATEDIFF(day, [PaidDateUtc], S.ShippingDate)>=3 then DATEDIFF(HOUR, dateadd(hh,48,S.[PaidDateUtc]),  S.ShippingDate)  
	         when datepart(dw,[PaidDateUtc])=7 and DATEDIFF(day, [PaidDateUtc], S.ShippingDate)>=2 then DATEDIFF(HOUR, dateadd(hh,24,S.[PaidDateUtc]),  S.ShippingDate)  
	    else DATEDIFF(HOUR, S.[PaidDateUtc],  S.ShippingDate) end AS ShippedHours   
	  ...
	  ,DATEADD(week, DATEDIFF(week,DATEADD(dd,-@@datefirst,'1900-01-01'),DATEADD(dd,-@@datefirst,convert(varchar(10), [PaidDateUtc], 120))), '1900-01-01') as weeks 
		...
		where S.ReferenceOrderId like '%ORD%'or S.ReferenceOrderId like '%OWE%' 
			 ) AS Tb
  ...
  select   months,
           sum(case when ReferenceOrderId like '%ORD%' then 1 else 0 end) as NumofShippedOrder,
  ...
  /*
  select   months,
           sum(case when ReferenceOrderId like '%ORD%' then 1 else 0 end)  as NumofrefundOrder,
  ...
  */
  ...
  from #temp1000
  where 
         Suppliers in ('BEUK','EABE','EKGB','EKHK','OWE','WAIT','OTHERS')
         and 
		[ShippingStatusId] not in (-1,40,41,57,58,59,60,61)
  group by months,Suppliers
  order by months,Suppliers
  


  --select top 100 * from #temp1000 where [ReferenceOrderId] like '%OWE%'
  --select top 100 * from #temp1000 where Suppliers like '%EKGB%' and  weeks='2017-07-10'
  --select * from  #temp1000



  -----group by serviceprovideid------------------------------------------------------------------------------------------------------------------

     select  #temp1000.[ServiceProviderId],
	        case when #temp1000.[ServiceProviderId]=0 then 'OWE'
			      when #temp1000.[ServiceProviderId]=32 then 'Yuantong'
			         else     max(SM.ServiceName) 
			     end as ServiceName,
	        months,
           sum(case when ReferenceOrderId like '%ORD%' then 1 else 0 end) as NumofShippedOrder,
           sum(ShipWeight) as TotalShipweight,
		   ...
		   sum(ShippingCost)/sum(ShipWeight) as CostperKg
  from #temp1000
  left join  [Tradewise].[dbo].[ShipmentMethod] SM on SM.[ServiceProviderId]=#temp1000.[ServiceProviderId]
  where #temp1000.[ServiceProviderId] in (0,1,3,4,5,6,8,9,16,17,19,20,21,22,25,26,30,31,32)
         and 
		 [ShippingStatusId]not in (-1,40,41,57,58,59,60,61)
  group by months,#temp1000.[ServiceProviderId]
  order by months,#temp1000.[ServiceProviderId]

  ------除此自外(0,1,3,4,5,6,8,9,16,17,19,20,21,22,25,26)还有（100）----------
