...
SELECT P.id
  ...
  WHERE [Paymenttime]>=@BegDate and [Paymenttime]<@EndDate 
		...
		--and b.name in ('Billingham')
		--and b.name in ('Zatchels','thisworks','Lily charmed','MARVEL')
		--and PI.ProductCode='THW075004'
		--and mallcode='WebAPI_51taouk.com'
		--and mallcode='TMall'
		--and mallcode='WebAPI_Daifa.51Parcel.com'
		--and PI.ProductCode in ('YMZA1','YMZA2','YMZA3','YMZA4','YMZC1','YMZC2','YMZC3','YMZC4','APT-A1','APT-A2','APT-A3','APT-A4','CAG-C1','CAG-C2','CAG-C3','CAG-C4','APT-AP1','APT-AP2','APT-AP3','APT90349','APT90337','APT79459','APT79224','APT106926','APT106925','APT106924','CAG79219','CAG57792','CAG80653','CAG85922')
...
where WRJ.PurchaseOrderNumber not like '%MPDCN%'
...
where ReferenceOrderId like '%MPDCN%' 
...
into #Report_1 from #temp500
 ...
 select [Paymenttime]
	...
	-- select [Brand],sum(Revenue) as sums from #revenue_temp1 group by [Brand] order by sums desc
	 ...
	 --where [Brand] like 'Billingham'-- '%HALCYON DAYS%' -- '%Zatchels%'
