...
if object_id('tempdb..#temp1000') is not null Begin
 ...
 set @enddate='2017-10-23' 
 set @begindate=dateadd(week,-4,@enddate)
 SET DATEFIRST 1
...
SELECT  
  ...
  FROM 
   (SELECT 
	  ...
	  ,DATEADD(week, DATEDIFF(week,DATEADD(dd,-@@datefirst,'1900-01-01'),DATEADD(dd,-@@datefirst,convert(varchar(10), [PaidDateUtc], 120))), '1900-01-01') as weeks 
		...
		where ReferenceOrderId like '%ORD%'or ReferenceOrderId like '%OWE%' 
		      ) AS Tb
  ...
  */
   ...
   select  Suppliers,weeks,
		   ...
		   sum(ShippingCost)/sum(ShipWeight) as CostperKg
  from #temp1000
  where Suppliers in ('BEUK','EABE','EKGB','EKHK','OWE','WAIT','OTHERS')
        and [ShippingStatusId] not in (-1,40,41,57,58,59,60,61)
  group by weeks,Suppliers
  order by weeks,Suppliers
  ...
  where #temp1000.[ServiceProviderId] in (0,1,3,4,5,6,8,9,16,17,19,20,21,22,25,26,30,31,32)
        and [ShippingStatusId] not in (-1,40,41,57,58,59,60,61)
  group by weeks,#temp1000.[ServiceProviderId]
  order by weeks,#temp1000.[ServiceProviderId]